...
ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
						 ...
						 if (GetOpenFileName(&ofn)==TRUE) {
							 ...
							 CreateProcess(fullLine.wc_str(), szFile, NULL, NULL, false, 0, 0, NULL, &si, &pi);
						 }
					 }
                break;
...
void PolycodePlayerView::handleEvent(Event *event){
	...
	case PolycodeDebugEvent::EVENT_ERROR:
		...
		AppendTextToEditCtrl(consoleTextArea, lineString.wc_str());
		AppendTextToEditCtrl(consoleTextArea, debugEvent->errorString.wc_str());
		ShowWindow(consoleHwnd, SW_SHOW);
		UpdateWindow(consoleHwnd);
		}
	break;
